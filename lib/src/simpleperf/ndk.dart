...
class Ndk {
  final String _ndkDir;

  Ndk({String path = 'tools/android-ndk'}) : _ndkDir = path;

  factory Ndk.fromEnvironment() {
    final path = Platform.environment['ANDROID_NDK'];
    if (path == null || !Directory(path).existsSync()) {
      throw ArgumentError('Set ANDROID_NDK to point to NDK');
    }
    final ndk = Ndk(path: path);

    // Validate presents of binaries.
    for (var path in [
      ndk._llvmSymbolizerBinary,
      ndk._llvmObjdumpBinary,
      ndk._readelfBinary,
      ndk._apiProfilerScript,
    ]) {
  ...
  Stream<String> objdump({
    ...
    List<String> options = const ['-d'],
  }) async* {
    final process = await Process.start(
        _llvmObjdumpBinary, ['--arch=arm64', ...options, object]);
  ...
  Future<void> apiProfilerPrepare() async {
    await _run(_apiProfilerScript, ['prepare']);
  }

  Future<void> apiProfilerCollect({required String app, String? outDir}) async {
    await _run(_apiProfilerScript, [
      'collect',
      '-p',
      app,
      if (outDir != null) ...['-o', outDir],
    ]);
  }
